...
## 2026-07-04 (19차) · 배포 중단(다른 세션에서 복구됨) 확인
- 18차 커밋에서 `app.js`가 418줄까지만 저장된 채 push되어 배포 사이트가 `SyntaxError`로 죽어있었으나, 별도 세션에서 이미 전체 내용으로 복구 완료된 상태를 확인함
- **교훈**: 탭 저장처럼 간단해 보이는 수정도 반드시 CLAUDE.md의 push 절차(Read로 전체 내용 확인 → python으로 새 clone에 직접 작성 → `node -c` 검증 → push)를 그대로 따를 것

## 2026-07-04 (18차) · 새로고침 시 탭이 "아이디어 수집"으로 초기화되는 문제 수정
- **원인**: `activeTab` 변수가 메모리에만 있고 저장되지 않아, 새로고침하면 항상 기본값 "idea"로 리셋됨
- **app.js** 수정 — `TAB_KEY="storyhelper_activeTab"` 추가, 탭 클릭 시 `localStorage`에 저장. 초기 로드 시 저장된 탭 값을 읽어 `activeTab` 초기값으로 사용(유효한 탭 이름일 때만), 해당 탭 버튼에 active 클래스 적용. 관리자 권한 상실로 admin 탭에서 강제 이동될 때도 저장값을 "idea"로 갱신
