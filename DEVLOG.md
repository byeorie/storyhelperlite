...
## 2026-09-16 (6) — 칸 블럭 추가 시 엉뚱한 단계에 생기는 버그

**요청**: 1막·2막·3막 같은 큰 섹션에서 칸 블럭 추가를 하면 섹션과 상관없는 곳에 생성됨.

**원인**: `uid()`가 `"p"+Date.now()+0~999 난수`라서 같은 1ms 안에 여러 개를 만들면 자주 겹쳤다.
플롯 구조 선택(단계 여러 개), [플롯 불러오기](아이디어마다 그룹+칸)처럼 한 번에 여러 id를 만들 때 중복이 생김
(아이디어 12개 불러오기 기준 약 24%). 단계 id가 겹치면 두 단계가 같은 칸을 공유하고, 그룹 id가 겹치면
다른 막의 아이디어 상자로 칸이 추가되며, 칸 id가 겹치면 이동 시 칸이 복제/유실된다.

**수정 (`app.js`)**
- `uid()` — 난수 6자리 + 이번 접속에서 발급한 id 기억(절대 중복 없음).
- `repairDuplicateIds()` 신설 — 이미 저장된 데이터의 중복 id(플롯 단계·그룹·칸·지문/대사·분기)를 고친다.
  겹친 그룹은 칸 덩어리를 단계 기준으로 나눠 각 그룹에 돌려준다. 글쓰기·콘티 탭을 그릴 때 실행, 고쳤으면 저장.
- `addSceneBlock` — 새 칸을 배열 맨 끝이 아니라 그 단계의 마지막 칸 바로 뒤에 넣음.

**확인**: Playwright로 단계 id·그룹 id·칸 id가 겹친 데이터를 넣고 글쓰기 탭을 열어 복구되는 것,
1막 [칸 블럭 추가]가 1막에 생기는 것 확인. 이전 기능(우클릭 블럭 생성, 터치 이동) 재확인.

## 2026-09-16 (5) — 지문·대사 다른 칸으로 이동 / 아이패드 블럭 이동 핸들

**요청**: ① 칸 블럭 안의 지문·대사 블럭을 다른 칸으로 옮길 수 있게 ② 아이패드에서 이동 핸들이 전혀 안 됨.

**수정**
1. `app.js` `sceneBlockEl` — 칸 블록 전체(`.scene-block`)에 지문/대사용 dragover/drop 추가. 예전에는 대상 칸의
   지문·대사 목록 위에 정확히 놓아야만 옮겨졌고, 지문·대사가 하나도 없는 칸은 목록이 `display:none`이라
   아예 넣을 수 없었다. 이제 칸 머리(제목줄) 위면 맨 앞, 그 아래면 맨 뒤로 들어간다.
2. `app.js` 자동 스크롤 코드 아래 "아이패드(터치·애플펜슬) 블록 이동" 추가 — iPad Safari에서는 HTML 드래그앤드롭이
   손잡이에서 시작되지 않으므로, 손잡이(`TOUCH_DND_HANDLES`)를 손가락/펜으로 끌면 pointer 이벤트로
   dragstart/dragenter/dragleave/dragover/drop/dragend를 직접 만들어 보낸다(기존 마우스용 처리 코드 재사용,
   자동 스크롤 포함). 마우스는 기존 방식 그대로. 효과 없던 `touchstart → draggable=true` 7곳 제거.
3. `style.css` — 손잡이에 `touch-action:none`, 길게누르기 메뉴·글자 선택 방지.

**확인**: Playwright(iPad 터치 에뮬레이션)로 지문 블록을 빈 칸의 머리 위로 끌어 놓았을 때 start→drop→end 순서로 이동됨.
...
**수정**
- `app.js` `openStoryboardFeedbackDrawModal` — 그리기 팝업 상단에 [학생 그림 위에 그리기](기본) / [빈 캔버스에 그리기] 전환 버튼.
  빈 캔버스는 학생 그림과 같은 크기의 흰 종이이며, 위에 학생 그림을 작은 참고 이미지로 보여준다(클릭 시 크게 보기).
  그린 뒤 방식을 바꾸면 확인 후 캔버스를 새로 만든다. `onSave(newKey, {blank})`로 방식을 넘긴다.
- `renderSbFeedbackBlocks` / 제출함 `onFeedback` / `submitStoryboardFeedback` — feedback 블록에 `blank:true`를 저장,
  학생·교수 화면에서 "피드백(빈 캔버스)"로 표시. DB·서버 변경 없음(feedback은 자유 JSON).
- `style.css` — `.sb-fb-mode`, `.sb-fb-ref` 추가.

## 2026-09-15 (10) — 글쓰기 첨삭 [반영] 시 내용이 사라지던 버그 수정

**증상**: 학생이 글쓰기 첨삭을 [반영]하면 내용이 모두 사라지고, 다시 [반영]해도 내용이 넘어오지 않음.
해당 학생은 글쓰기에만 내용을 적었고 **플롯은 비어 있었다**.

**원인 3가지가 겹침**

1. **플롯이 비면 글쓰기 탭이 안 열림** — `rWrite()`는 `!pd.structure || !pd.sections.length`면
   "먼저 플롯 생성에서 플롯 구조를 만들어 주세요"만 그리고 early-return한다. 반영 직후
   `feedbackPage=null; render()`로 글쓰기 탭에 돌아오면 이 화면만 보여 **내용이 통째로 사라진 것처럼 보였다**.
2. **빈 첨삭 텍스트가 원문을 덮어씀** — `bl.items=parseFeedbackTextToItems(fb.text||"")`는 빈 텍스트에
   `[]`를 돌려줘 그 칸의 내용을 실제로 지웠다(데이터 손실).
3. **블록 id를 못 찾으면 조용히 아무것도 안 함** — `blocks.find(b=>b.id===fb.id)`가 실패하면 그냥 넘어가서,
   블록이 사라진 뒤에는 **몇 번을 [반영]해도 내용이 넘어오지 않았다**.

**수정** (`app.js`)

- `applyWriteFeedback(feedback, memos, data)` 신설 — 글쓰기 반영 로직을 분리하고 제출물 원본(`data`)을 함께 받는다.
  - 로컬에 블록이 없으면 제출물의 블록(제목·섹션·그룹 포함)을 **되살려서** 반영한다.
  - 첨삭 텍스트가 비어 있으면 **제출 당시 원문**으로 채운다(빈 값으로 덮어쓰지 않음).
- `ensureWriteSections(subBlocks)` 신설 — 블록이 속한 플롯 섹션이 없으면 제출물의 `sectionName`으로 섹션을
  되살리고, 그래도 없으면 "글쓰기" 섹션 1개를 만든다. 구조가 비어 있으면 `structure="custom"`으로 채워
  글쓰기 탭이 열리게 한다(`PLOT_STRUCTURES`에 없는 키라 플롯 탭에는 "사용자 구조"로 표시됨).
- `applyFeedbackToProject(type, feedback, memos, data)` — 인자에 `data` 추가, write 분기는 위 함수 호출로 교체.
- 첨삭 보기의 [반영] 버튼 — `applyFeedbackToProject(sub.type, sub.feedback, memos, sub.data)`로 원본을 전달.

**결과**: 플롯이 비어 있어도 첨삭 내용이 글쓰기 탭에 그대로 들어오고 바로 편집할 수 있다.

**추가 수정 — "저장 중…" 배너가 안 사라지던 문제** (`auth.js` · `doServerSave`):
D1 쓰기 절약을 위해 `payload === lastSavedJson`이면 요청을 안 보내고 early-return하는데, 이 경로에서만
`showSaveToast()`를 부르지 않아 `save()`가 띄운 "저장 중…"이 영영 남아 있었다. → 이 경로에서도 "저장됨"으로 닫도록 수정.

참고: 첨삭 [반영]을 눌러도 아무 내용이 안 바뀌면(= 위 버그로 블록을 못 찾던 상황) 바로 이 경로로 들어온다.
따라서 **"저장 중…"이 계속 떠 있는 것은 반영이 실제로 아무것도 하지 않았다는 신호**였다.

**수정 파일**: `app.js`, `auth.js`

**배포 확인 방법**: 배포가 끝났는지는 브라우저에서 사이트를 연 뒤 주소창에 콘솔로
`typeof applyWriteFeedback`을 실행해 `"function"`이 나오면 새 코드다(`"undefined"`면 아직 예전 코드).
`_headers`에 캐시 설정이 없어 Cloudflare 기본값(`max-age=0, must-revalidate`)이 적용되므로,
배포만 끝나면 학생 브라우저는 새로고침으로 바로 새 코드를 받는다.
...
- 모달: `#newAssignDueTimeOn` 체크박스. 끄면 시·분 select가 `disabled`.
- 저장: 켜짐 → `T${시}:${분}:00`, 꺼짐 → `T23:59:59`(예전에 날짜만 고르던 때와 같은 뜻).
- 읽기: `unixToTimeParts` → **`dueTimeState(sec)`**로 교체. 23:59 이상이면 `{on:false}`로 돌려주므로
  예전에 만든 과제(23:59:59 저장)도 "시간 미지정"으로 그대로 열린다.
- 표시: **`fmtDue(sec)`** 신규 — 23:59면 날짜만, 아니면 `fmtDateTime`. 교수·학생 과제 목록 2곳에 적용.
- `style.css`: `.due-time-on`, `.due-row select:disabled{opacity:.45}`.
- 주의: 23:59를 "시간 미지정"의 표식으로 쓰고 있으므로, 10분 단위 목록에 59분을 넣으면 안 된다.

## 2026-09-15 (6) — 제출기한에 시각(24시간 · 10분 단위) 추가

과제 등록/설정 변경 모달의 제출기한을 **날짜 + 시 + 분**으로 바꿨다. 예전에는 날짜만 고를 수 있어서
무조건 그날 `23:59:59`로 저장했다(수업 시간에 맞춰 마감할 수 없었음).

### `app.js`
- `unixToTimeParts(sec)` 신규 — 저장된 기한에서 시·분을 꺼내되 **분은 10분 단위로 내림**
  (예전에 저장된 23:59:59는 23:50 칸이 선택된다 → 그대로 저장하면 9분 앞당겨지니 주의).
  기한이 없으면 기본값 `23:50`(`DUE_DEFAULT_H`/`DUE_DEFAULT_M`).
- 모달: `<input type="date">` + `#newAssignDueH`(00~23시) + `#newAssignDueM`(00·10·20·30·40·50분).
  `<input type="time">`을 쓰지 않은 이유 — 브라우저 언어에 따라 오전/오후로 나오고 10분 단위 강제도
  브라우저마다 달라서, 24시간·10분 단위를 확실히 하려고 select 두 개로 했다.
- 저장: `new Date(`${dueStr}T${dueH}:${dueM}:00`)` → unix초. 날짜가 비어 있으면 예전처럼 null.
- 표시: 교수 과제 목록·학생 과제 목록의 `제출기한 fmtDate(...)` → `fmtDateTime(...)` 2곳.
- 사용법 문구에 "(날짜 + 시각 · 24시간 10분 단위)" 추가.

### `style.css`
- `.due-row`(날짜는 늘어나고 시·분 select는 제 너비) 추가.

서버(`professor-assignment(s).js`)는 원래 unix초를 그대로 받으므로 고칠 것이 없다.

## 2026-09-15 (5) — 자동 새로고침 주기 20초 → 1분

`app.js` `PROF_AUTO_SEC=60`. 화면 문구는 `PROF_AUTO_LABEL="1분"`으로 따로 두었다
(초 단위를 그대로 쓰면 "자동 (60초)"로 나오므로). 사용법 안내 문구도 함께 고침.

## 2026-09-15 (4) — 제출함 목록에 제출 시각(시:분)까지 표시

교수 제출함 줄의 `제출 2026.09.15` → `제출 2026.09.15 12:07`.
`app.js` `loadProfAssignmentFolder()`의 `fmtDate(s.submitted_at)` → `fmtDateTime(...)` 한 곳.
같은 날 여러 번 제출·재제출한 경우 날짜만으로는 순서를 알 수 없어서.
([이전 버전] 팝업의 차수 목록은 원래부터 `fmtDateTime`을 쓰고 있었다.)
...
- 대사 문단 사이 빈 줄을 **2개 → 1개**로 줄였다.
- 출력 글꼴을 **맑은 고딕 10pt**로 고정했다. docx 라이브러리의 `size`는 half-point 단위라
  10pt = `20`. `font`에 문자열을 주면 rFonts의 ascii/hAnsi/eastAsia/cs에 모두 같은 이름이
  들어가므로 한글도 맑은 고딕으로 나온다.
- 빈 줄 문단도 빈 `TextRun`에 같은 글꼴·크기를 줘서 줄 간격이 들쭉날쭉하지 않게 했다.
- 문서 기본 스타일(`styles.default.document.run`)에도 같은 글꼴·크기를 넣어 두었다.

## 2026-09-15 (1) — 콘티 캔버스 실치수 변경 · 프리사이즈 추가

콘티 칸의 캔버스 해상도를 웹툰 원고 실치수로 바꾸고, 세로를 직접 정하는 "프리사이즈"를 추가했다.

### 1) 칸 크기 (`app.js` `SB_SIZES`)
| 칸 | 이전(캔버스) | 변경 |
|---|---|---|
| 큰 칸 | 350 × 500 | **690 × 1500** |
| 중간 칸 | 350 × 350 | **690 × 750** |
| 작은 칸 | 350 × 250 | **690 × 500** |
| 프리사이즈 | (없음) | **690 × 직접 지정 (200~3000px)** |

- 예전에는 캔버스를 350px로 만들고 화면 문구만 2배(`SB_SIZE_LABEL_SCALE=2`)로 적었다.
  이제는 캔버스 해상도 자체가 위 숫자 그대로여서 표시 배율을 **1**로 되돌렸다.

### 2) 프리사이즈
- 크기 값은 `"free:1200"` 꼴의 문자열로 저장한다(기존 `"large"/"medium"/"small"`과 섞이지 않음).
- `sbFreeKey(h)` 값 만들기(200~3000px으로 잘라 맞춤), `sbSizeOf(key)` 값 → `{w,h,label}` 해석,
  `sbNormalizeSize(key)` 불러온 데이터 정리 — 세 함수를 새로 두고,
  기존의 `SB_SIZES[key]||SB_SIZES.medium` 조회를 모두 `sbSizeOf(key)`로 바꿨다
  (콘티 칸 미리보기 · 그리기 페이지 · 첨삭 팝업 · 첨삭 블록 4곳).
- 크기 고르기 팝업(`openSizePicker`)에 세로 입력칸 + [이 크기로] 줄을 붙였다(Enter로도 선택).
- `style.css`: `.sb-size-free` / `.sb-size-free-row` / `.sb-size-free-input` 추가.

### 3) 미리보기 배율
- 캔버스가 690×최대 3000px이라 그대로 걸면 화면에서 너무 커서, 미리보기는 `sbThumbScale()`로
  기본 1/2 축소하고 아주 긴 프리사이즈는 세로 900px 안쪽으로 더 줄인다(`sbThumbStyle()`).
  콘티 칸 미리보기와 첨삭 화면 이미지가 같은 함수를 쓴다.

### 4) 업로드 용량 상한
- `sbMaxBytes()`를 `sizeKey==="large"` 비교 대신 **칸 넓이 기준**(`w*h >= 600,000` → 500KB, 그 외 300KB)
  으로 바꿨다. 프리사이즈도 자동으로 알맞은 상한을 받는다. 서버 상한 600KB는 그대로.

### 5) 예전 데이터
- `large/medium/small`로 저장된 기존 콘티는 그대로 열리며, 캔버스만 새 치수로 커진다
  (이미 그린 그림은 이미지라서 그대로 보인다). 알 수 없는 값은 중간 칸으로 읽는다.

## 2026-09-14 (11) — 사용법 안내 갱신 · 설정(캐릭터/배경/사건) PDF 내보내기

### 1) "사용법" 안내 모달(GUIDE_SECTIONS)을 현재 기능에 맞게 전면 갱신
2026-08-20에 쓰인 뒤로 손대지 않아 실제 화면과 어긋난 설명이 많았다. `app.js`의 `GUIDE_SECTIONS`를 교체.
- **계정 설정**: "프로필 아이콘 → 개인정보 수정 · 등록 코드 · 비밀번호 변경 3개" → 실제로는
  개인정보 수정 · 비밀번호 변경 · 로그아웃이고, 등록 코드는 2026-09-02에 왼쪽 메뉴
  [수업 코드 입력](`#classCodeMenuBtn`)으로 옮겨졌다. 그에 맞게 수정.
- **내보내기 목록**: 콘티 .jpg(2026-08)와 이번에 추가한 설정 .pdf를 빠짐없이 적고, 모바일 더보기(⋮) 안내 추가.
- **아이디어 수집**: 2026-09-02부터 작품 단위가 아니라 **계정 단위 공유**(샘플 작품만 예외)임을 명시.
- **플롯 생성**: "영웅의 여정 12단계" 고정처럼 읽히던 문장을 3막/5막/영웅의 여정/8단계 원형/액자/비선형/
  옴니버스 중 **선택**으로 수정(`PLOT_STRUCTURES` 8종).
- **캐릭터/배경/사건**: 역할의 변화·에니어그램 화살표·인물의 변화 3단계·외모 세분화(9/14), 관계,
  프로필 사진/캐릭터 이미지(시트), 용어사전, 회차 일지, 커스텀 항목 추가(9/02), 오른쪽 미리보기 탭을 반영.
- **글쓰기/콘티**: 섹션 블럭(그룹)+칸 블럭 구조(9/10~9/11), 그리기 엔진(레이어 3장·브러시 3종·채우기·
  확대/축소·가이드선·필압·Ctrl+Z·PNG 저장, 9/14)을 반영.
- **과제 제출(학생)**: 수업 단위 드롭다운(9/08), 과제 종류 지정(9/12), 첨삭 전 재제출 덮어쓰기(9/14),
  알림 토스트와 접기(9/11·9/14), 평가(총평)와 메모 색 구분(9/10·9/11), 콘티 피드백 비교 보기를 추가.
- **교수 기능**: 수업 코드 발급/크게 보기, 수업 정보 5항목(9/01), 상단 [전체 학생 명단]·[수업 미지정 과제]
  버튼과 드래그 순서(9/14), 과제 종류·과제 설정 변경(9/12), [과제 확인](9/08), 평가 입력과 [피드백 전달] 후
  제출함 복귀(9/14), 콘티 위에 직접 그리는 첨삭, 제출물 학생별 PDF 일괄(zip) 다운로드를 반영.
- **샘플 데이터** 섹션 신설(2026-09-02부터 학생에게도 보임).

### 2) [내보내기] → **설정 출력 (.pdf)** 신규
캐릭터 설정 · 배경 설정 · 사건 설정을 한 PDF로 묶어 내려받는다. 캐릭터는 **프로필 사진과 캐릭터
이미지(시트)를 함께** 싣는다.
- `index.html`: 내보내기 메뉴에 `#topExportSetting`("설정 출력 (.pdf)") 추가 — 대사만 출력과 콘티 출력 사이.
  내보내기 버튼 title도 갱신.
- `app.js`(`exportStory` 바로 앞)에 신규 함수 묶음:
  `SETTINGS_PDF_CSS`, `waitForImagesIn`, `settingsPdfRows/Head/SubHead/CustomRows`,
  `settingsPdfCharHtml`, `settingsPdfBackgroundHtml`, `settingsPdfEventHtml`,
  `appendSettingsBlockToPdf`, `settingsPdfToast`, `exportSettingsPdf`.
- 방식은 제출물 PDF 일괄 다운로드와 동일 — 화면 밖(`left:-99999px`) 760px 폭 div에 인쇄용 HTML을 그려
  `html2canvas`(scale 2)로 캡처하고 `jsPDF`(A4)에 담는다. 라이브러리는 기존 `ensureBulkPdfLibs()` 재사용
  (처음 눌렀을 때만 CDN에서 로드).
- **블록 하나(캐릭터 한 명 / 배경 / 사건)가 새 페이지에서 시작**하고, 길면 그 안에서만 여러 페이지로 잘린다.
- 이미지는 `waitForImagesIn()`으로 **다 뜬 뒤에** 캡처한다(안 기다리면 빈 칸으로 찍힘, 8초 타임아웃).
  시트 이미지는 `charSheetUrl(ch)`(R2 `/api/storyboard-image?key=`), 프로필 사진은 데이터URL.
- 항목은 `CHAR_FIELDS`(이름 제외, `charFieldValue`로 roleArcs 같은 배열도 한 줄로) / `BG_FIELDS`(src에 따라
  `P.world`·`P.background`) / `EVENT_FIELDS`를 쓰고, 값이 빈 항목은 건너뛴다. 여기에 더해 인물 관계,
  용어사전(`P.world.glossary`), 회차 일지(`P.event.log`), **커스텀 항목(`customFields`)** 까지 포함한다
  (기획서 .docx 내보내기가 커스텀 항목을 못 담는 것과 달리, 이 PDF는 담는다).
- 진행 표시는 메뉴 버튼 대신 상단 저장 배너(`#saveToast`)를 빌려 쓴다 — 메뉴 버튼은 안에 SVG 아이콘이
  있어서 `textContent`를 바꾸면 아이콘이 지워진다(기존 콘티 .jpg 버튼의 알려진 문제라 따라 하지 않음).
- 파일명 `작품명_설정.pdf`(윈도우 금지 문자는 `_`).

### 검증
- `node --check app.js` 통과.
- Chromium(Playwright)으로 실제 `index.html`을 띄워: 사용법 모달 10개 섹션 정상 렌더(콘솔 오류 0건),
  캐릭터 1명(프로필 사진 + 시트 이미지 + 역할 변화 + 커스텀 항목) · 배경(용어사전 포함) · 사건(회차 일지 포함)
  으로 설정 PDF 생성 → 4페이지(캐릭터 2p · 배경 1p · 사건 1p), 이미지와 표가 정상 출력되는 것을 확인.
...
### 6) 그룹 안 칸 블록의 배경/캐릭터를 제목 자리(머리줄)로
제목 칸이 빠지면서 생긴 빈 공간에 배경·캐릭터 메모를 넣어 한 줄을 아낀다.
- 메타 줄 생성 코드를 `sceneMetaRow(bl)` 함수로 분리(내용 없으면 null).
- 그룹 안 칸: `.scene-title-spacer` 안에 넣어 머리줄에 표시. 그룹 밖 낱개 칸: 기존대로 제목 아래 줄.
- `style.css`: 머리줄 안에서는 2열 그리드 대신 flex-wrap 한 줄로 흐르게.

### 7) 아이디어(섹션 블럭) 삭제
그룹 머리에 휴지통 버튼 추가 — `deleteBlockGroup(gid)`가 그룹과 그 안의 칸 블록을 함께 삭제(확인창에 칸 개수 표시).
"그룹 해제"(칸은 남기고 상자만 없앰)와는 별개 기능. 플롯 탭의 아이디어 원본은 남으므로 [플롯 불러오기]로 재사용 가능.
`style.css`: `.wg-actions button.wg-del:hover` 빨간색.
...
사라지는지 알아봐줘." → 원인 조사 후 수정.

- **원인 1 (핵심)**: `app.js` 학생용 그리기 팝업 `openDrawModal`의 저장 버튼이 **업로드 결과를 기다리지 않고**
  `saveStoryboardBlob(...)` 호출 직후 곧바로 팝업(캔버스)을 제거했다. 업로드가 실패하면(인터넷 끊김,
  로그인 세션 만료 401, 용량 초과) 알림만 뜨고 그림은 이미 사라져 되살릴 수 없었다. 교수 첨삭용
  `openStoryboardFeedbackDrawModal`은 `await` 후 실패 시 팝업을 유지하도록 되어 있었는데 학생용에만
  이 방어가 빠져 있었음.
  → `saveStoryboardBlob`이 성공/실패를 boolean으로 돌려주도록 바꾸고, 저장 버튼은 **성공했을 때만** 팝업을
  닫는다. 실패하면 버튼이 다시 살아나고 그림은 화면에 그대로 남는다.
- **원인 2 (용량)**: `compressCanvasToLimit`이 **화질만** 낮추고(0.3에서 포기) 해상도는 줄이지 않아, 큰 칸에
  색을 꽉 채워 그리면 목표(300KB)를 못 맞춘 blob이 그대로 업로드 → 서버 상한 400KB에 걸려 400 거부 → 소실.
  → 화질을 낮춘 뒤에도 모자라면 **그림 크기(해상도)까지 단계적으로 줄이도록** 수정. 상한도 상향:
  `sbMaxBytes()` 신설 — **큰 칸 500KB**, 작은/중간 칸 300KB. 서버(`functions/api/storyboard-image.js`)의
  `MAX_BYTES`는 400KB → **600KB**로 올려 여유를 둠. 교수 첨삭용 팝업도 같은 기준을 쓴다.
- **원인 3 (블록 참조 유실)**: 그림을 그리는 동안 서버에서 작품이 다시 불러와지면(`auth.js loadFromServer`가
  `DB`를 통째로 교체 — 연결이 끊겼다 3초 뒤 재시도로 복구되는 경우 등) 손에 든 블록 객체가 버려진 옛
  데이터가 되어, 저장해도 **아무 오류 없이** 반영되지 않았다.
  → `liveWriteBlock(bl)` 신설: 같은 id의 블록을 현재 작품에서 다시 찾아 그쪽에 저장. 못 찾으면 방금 올린
  이미지를 정리하고 "새로고침 후 다시" 안내하며 팝업을 닫지 않는다.
- **원인 4 (서버 저장 안 되는 상태)**: "서버 연결 실패" 상태에서는 `serverSaveReady()`가 false라 서버 저장을
  통째로 건너뛴다(덮어쓰기 사고 방지). 이때 그린 콘티는 그 브라우저에만 남아 다른 컴퓨터에서는 사라진
  것처럼 보였는데 아무 안내가 없었다. → 저장 후 이 상태면 명시적으로 알려 준다.
- **원인 5 (기존 그림 미로딩)**: [다시 그리기]에서 기존 이미지를 못 불러온 채 저장하면 원본이 빈 캔버스로
  덮여 사라졌다. → 불러오기 실패를 감지해 즉시 알리고, 그래도 저장하려 하면 한 번 더 확인받는다.
- **부수 개선**: 콘티 저장은 0.6초 디바운스를 기다리지 않고 `forceSaveToServer()`로 즉시 서버에 올린다
  (저장 직후 탭을 닫아 유실되는 경우 방지).
- **수정 파일**: `app.js`, `functions/api/storyboard-image.js`. node --check 구문 검증 완료. DB 마이그레이션 불필요.

## 2026-09-03 (2) — 학생 단체 회원가입 시 "서버..." 오류로 가입 막히는 문제 수정

**요청**: "학생들이 회원가입하는데 몇명이 서버 어쩌고 하는 메세지가 뜨면서 가입이 안됐어."

- **원인**: `functions/api/signup.js`의 가입 스팸 방지 제한(2026-08-20 추가)이 **같은 IP당 1시간에 10건**으로
  걸려있었음. 학교 와이파이는 보통 같은 공인 IP를 씀 — 수업 중 학생들이 한꺼번에 가입하면 반 인원이 10명만
  넘어도 11번째부터 막혀서 "서버에 연결할 수 없습니다"/"너무 많이 시도했습니다" 오류가 뜸.
- **functions/api/signup.js** 수정 — 처음엔 IP당 1시간 10건 → 60건으로 완화했다가, "동시다발적으로 여러
  학교에서 여러 명이 가입할 수 있다"는 사용자 설명에 따라(여러 학교=여러 IP에서 동시에 몰릴 수 있어 IP당
  건수를 아무리 늘려도 근본 해결이 안 됨) **가입 IP 횟수 제한 자체를 완전히 제거**함(checkRateLimit/
  clientIp import도 정리). 비밀번호 길이·아이디 형식·이메일 형식 검증과 "아이디 중복 불가"는 그대로 남아
  최소한의 방어는 유지됨.
- node --check로 구문 검증 완료. DB 마이그레이션 불필요.

## 2026-09-03 — 다른 컴퓨터에서 로그인하면 데이터가 사라지는 문제(같은 날 안에도 발생) 수정

**요청**: "같은 계정인데, 다른 컴퓨터에서 로그인 하면 기존 데이터가 사라지는 것 같은데."
(오늘 하루 안에 컴퓨터 A에서 편집 후 컴퓨터 B에서 로그인했더니 사라진 사례 — 3/1·9/1 정기 초기화와는
무관, 코드 조사로 실제 원인 2가지를 확인해 수정함.)

1. **(app.js/auth.js) 마지막 편집이 서버에 반영되지 않은 채 유실**: 글자 입력마다 로컬(localStorage)엔
   즉시 저장되지만, 서버 저장(`saveToServer`)은 마지막 입력 후 0.6초 디바운스 후에 나갔다. 그 0.6초 안에
   탭을 닫거나 다른 컴퓨터로 넘어가면 그 편집 내용이 서버에 전혀 안 올라간 채 끝나버렸음(창 종료 시
   대기 중이던 저장을 마무리하는 로직이 아예 없었음). **auth.js**에 `flushPendingServerSave()` 신설 —
   `visibilitychange`(hidden, 탭 전환 등 페이지가 살아있는 대부분의 경우)와 `pagehide`(실제 탭 종료,
   `keepalive:true` fetch로 최대한 보장 — 브라우저별 대략 64KB 제한은 알려진 한계) 두 시점에 대기 중인
   디바운스 저장을 즉시 실행하도록 함. `saveToServer()`/`forceSaveToServer()`가 실행 시점에
   `saveToServerTimer`를 다시 `null`로 되돌리도록 고쳐서 "지금 대기 중인 저장이 있는지"를 정확히
   판단할 수 있게 함.
2. **(auth.js, 더 심각) 서버에 데이터가 없다고 판단되면 로컬 백업까지 덮어써 지움**: `loadFromServer()`는
   서버 응답이 비어있으면(2026-08-18 "다른 계정 테스트 데이터가 새 계정에 섞이는 문제" 수정 이후) 무조건
   빈 작품으로 새로 시작하고 그 빈 내용을 로컬 저장소에도 즉시 덮어썼다. 1번 문제 등으로 서버 동기화가
   아직 안 된 "이 계정 자신의" 로컬 데이터까지 이 로직에 휩쓸려 지워지는 게 실제 원인이었음. **app.js**
   `save()`가 로그인 상태로 저장할 때마다 새 키 `LS_OWNER_KEY`(`storyhelper_v1_owner`)에 현재 계정
   아이디를 남기도록 하고, **auth.js** `loadFromServer()`의 "서버에 데이터 없음" 분기에서 이 태그가
   지금 로그인한 계정과 일치하면(=이 컴퓨터 로컬 데이터가 이 계정 자신의 것으로 확인됨) 지우지 않고
   그대로 서버로 복구 업로드하도록 수정. 태그가 없거나 다른 계정이면 기존처럼(2026-08-18 보호 로직)
   빈 작품으로 새로 시작함 — 그 경우의 동작은 그대로 유지.
- node --check로 app.js/auth.js 구문 검증 완료.
- DB 마이그레이션 불필요(서버 스키마 변경 없음, 브라우저 localStorage에 새 키 하나 추가되는 것뿐).
- ⚠️ **오늘 이미 벌어진 데이터 소실 자체는 이 수정으로 복구되지 않음** — 코드 배포 이후부터 같은 문제가
  재발하지 않도록 막는 수정임. 원래 컴퓨터의 브라우저 로컬 저장소에 아직 남아있는 내용이 있는지 사용자에게
  오프라인 상태로 확인 후 [내보내기 → 작품 파일(.story)]로 백업하도록 별도 안내함.

## 2026-09-01 (4) — 수업명 변경 + 학교이름/분반/요일/시간 입력 기능 추가

**요청**: "수업 이름 변경 가능하도록 해주고, 수업명과 별개로 학교이름, 분반, 수업 요일, 시간을 입력할 수
있도록 해줘."

- **schema.sql**: `classes` 표에 `school_name`, `section`, `class_day`, `class_time` 컬럼 4개 추가
  (모두 선택 입력, NULL 허용) — **DB 마이그레이션 대상**.
- **functions/api/professor-classes.js**: 수업 만들기(POST)에서 학교이름/분반/요일/시간도 함께 받아
  저장. 수업 목록(GET)에도 네 컬럼 함께 반환.
- **functions/api/professor-class.js**: 수업 상세(GET)에도 네 컬럼 반환. 기존엔 수업명만 바꿀 수 있던
  수정(POST)이 이제 학교이름/분반/요일/시간도 함께 받아 한 번에 저장(수업명만 필수, 나머지는 선택).
- **app.js**: 수업 만들기 모달에 학교이름(텍스트)/분반(텍스트)/수업 요일(월~일 선택)/수업 시간(텍스트,
  예: "14:00~16:50") 입력칸 추가. 수업 목록 각 카드와 수업 상세 제목 옆에 연필 아이콘(정보 수정) 버튼을
  새로 추가해 언제든 수업명 포함 다섯 항목을 함께 수정할 수 있게 함. 수업 목록 카드와 상세 제목에
  "OO대학교 · 2분반 · 화요일 14:00~16:50" 형태로 표시(`classMetaLine`, 값이 없는 항목은 자동 생략).
- node --check로 app.js/professor-class.js/professor-classes.js 구문 검증 완료.

## 2026-08-24 (2) — "학생 관리" 독립 탭 제거, [수업 관리] 안의 "전체 학생 명단"으로 완전 흡수

**요청**: "학생 관리도 수업 관리 안에 포함 시키고, 학생 관리 페이지는 없애줘."

- **index.html** 수정 — 사이드바 교수 메뉴에서 "학생 관리" 버튼(data-tab="profStudents") 삭제. 이제
  교수 메뉴는 "수업 관리" 하나만 남음.
- **app.js** 수정 — 기존 독립 함수였던 `rProfStudents`를 `renderProfRosterView(container)`로 바꿔
  [수업 관리] 안에서 재사용하도록 함. `rProfClasses`의 렌더러 등록·탭 가드에서 profStudents 제거.
  수업 목록(`renderProfClassList`) 맨 위에 삭제 불가한 "전체 학생 명단" 항목(data-class="roster")을
  추가 — 클릭하면 내 코드로 가입한 전체 학생 명단(예전 학생 관리 페이지와 동일한 내용)을 보여줌.
  기존 "전체 학생 (수업 미지정)" 항목은 "전체 학생 명단"과 이름이 겹쳐 혼동되지 않도록 **"수업 미지정
  과제"**로 이름 변경(동작은 동일). `rProfClassDetail`에 classId==="roster" 분기 추가(탭 없이 명단만
  표시). 수강생 추가 안내 문구의 "[학생 관리]" 언급을 모두 "[전체 학생 명단]"으로 교체. 사용법
  안내(usage guide) 모달의 "교수 계정 기능" 섹션 텍스트도 이 구조에 맞게 갱신.
- node --check로 app.js 구문 검증 완료(첫 시도에서 주석 블록을 잘못 닫아 `}` 밖으로 한글 문장이 새는
  구문 오류가 났던 것을 발견해 수정 — 배포 전에 잡아냄).
- DB 마이그레이션 불필요(테이블 구조 변경 없음, 화면 구성만 변경). autopush.bat만 실행하면 됨.
...
입력창 옆으로 넘어가는 경우 줄넘김을 해서 보이도록 하고, 아래로 넘어가는 경우 입력창 크기가 아래로
넓어지도록 해줘."

- **여러 줄 textarea (기획서/캐릭터/배경/사건/글쓰기 등 앱 전체)**: 지금까지는 `.sub-textarea`(글쓰기
  탭)에만 자동 높이 조절이 적용돼 있어서, 다른 탭의 textarea(시놉시스, 캐릭터 배경, 사건 설명 등)는
  내용이 길어지면 위아래로 스크롤해야 했음. `app.js`에 전역 `input` 이벤트 리스너를 추가해 앱의 모든
  textarea에 자동 높이 조절(`autoGrowTextarea`)이 적용되도록 하고, `render()` 끝에서 화면에 그려진 직후
  (저장된 긴 글도 포함) 한 번 더 높이를 맞춰줌. `style.css`에서 `textarea{resize:vertical}`를
  `resize:none`으로 바꿔 수동 크기조절 핸들을 없앰(자동으로 늘어나므로 충돌 방지) — 화면 내에서 실제로
  스크롤 없이 늘어나는 것을 Playwright로 확인(시놉시스에 긴 문단 입력 → 새로고침해도 저장된 긴 글이
  스크롤 없이 펼쳐져 보임).
- **한 줄짜리 `<input type="text">` (좌우 스크롤 문제)**: `<input>`은 원래 구조적으로 줄바꿈이 불가능해서
  (HTML 자체의 한계) 내용이 길어지면 옆으로 스크롤될 수밖에 없음. 이야기가 길어질 만한 항목들을
  골라 자동 줄바꿈·자동 높이 늘어남이 가능한 한 줄짜리 textarea(새 CSS 클래스 `.ta-line`)로 바꿈 —
  아이디어 입력창(`ideaNewInput`), 캐릭터 설정의 직업/신분·소속/세력·목표·결함·강점·비밀·매력
  포인트·좋아하는 것·싫어하는 것, 배경 설정의 전체 분위기/톤, 세계관 용어사전의 한 줄 정의, 사건
  설정의 사건명·사건 관리(회차 일지)의 사건명/관련 인물, 기획서 작성의 장르. (이름/나이/성별/태그처럼
  짧게 끝나는 항목은 기존 한 줄 입력창 그대로 둠.) `.ta-line`에는 Enter 키를 눌러도 줄바꿈이 생기지
  않도록 막아서(원래 한 줄 입력창이었던 느낌 유지, 아이디어 입력창의 "Enter로 등록"과도 충돌 없음)
  전역 keydown 리스너로 처리.
- Playwright로 데스크톱(900px)에서 긴 텍스트 입력 시 좌우 스크롤 없이 줄바꿈되고(scrollWidth===
  clientWidth) 높이가 자동으로 늘어나는 것(스크롤 없이 scrollHeight ≈ clientHeight)을 확인. 새로고침 후
  기존에 저장된 긴 글도 스크롤 없이 펼쳐져 보이는 것, 캐릭터 이름 필드는 여전히 일반 입력창(한 줄
  고정)인 것도 함께 확인함.

## 2026-08-22 (3) — 모바일 상단바 정리: 새 작품/저장/불러오기/내보내기를 롤링 메뉴로, 로고 문구 숨김

**요청**: "모바일 버전에서 상단탭 정리해줘. 새로만들기/저장/불러오기/내보내기는 하나의 롤링메뉴로 변경해주고,
'스토리 가이드' 문구는 안보이게 해서 상단탭의 공간을 확보해줘."

- `index.html`: 새 작품(`newProjBtn`)/저장(`manualSaveBtn`)/불러오기(`topImportBtn`)/내보내기(`mb-export`)
  4개 버튼을 새 컨테이너 `#mbMoreGroup`으로 묶고, 그 앞에 모바일 전용 "더보기"(⋮) 버튼 `#mbMoreBtn`을
  추가. 실행취소/다시실행은 요청대로 그룹에 포함하지 않고 그대로 둠. 각 버튼에 라벨 텍스트
  (`<span class="mb-icon-label">`)를 추가(데스크톱에서는 숨김, 모바일 드롭다운에서만 표시).
- `style.css`:
  - 데스크톱: `.mb-more-group{display:contents}`로 그룹 래퍼 자체를 레이아웃에서 지워서 기존과 똑같이
    보이게 하고, `order` 값(새 작품1→실행취소2→다시실행3→저장4→불러오기5→내보내기6→구분선7)으로 원래
    시각적 순서를 그대로 유지(DOM 순서가 바뀌어도 화면에 보이는 순서는 원래와 동일 — Playwright로 각
    버튼의 x좌표가 정확히 36px 간격으로 원래와 동일함을 확인).
  - 모바일(768px 이하): `#mbMoreBtn`만 보이고 `#mbMoreGroup`은 평소 숨겨져 있다가, 클릭하면
    `body.mb-more-open` 클래스로 세로 드롭다운 패널(라벨 포함)로 나타남. 안의 "내보내기"를 누르면 기존
    내보내기 하위메뉴가 그 아래 이어서 열림(별도 로직 없이 기존 `topExportBtn`/`topExportMenu`를 그대로
    재사용).
  - `.topbar h1`을 모바일에서 `font-size:0`으로 만들어 "스토리 가이드" 문구만 숨김(아이콘은 px 고정
    크기라 영향 없이 그대로 보임) — 상단탭(작품 탭)과 "다른 작품 열기" 영역에 공간을 확보.
- `app.js`: `#mbMoreBtn` 클릭 시 `body.mb-more-open` 토글(다른 곳 클릭 시 자동 닫힘 — 기존 내보내기
  메뉴를 닫던 전역 클릭 리스너에 같이 얹음). 새 작품/저장/불러오기 버튼은 원래 로직 그대로이며, 클릭하면
  이벤트가 전역 리스너까지 버블링되어 더보기 메뉴도 함께 자동으로 닫힘.
- Playwright로 데스크톱(1280px, 아이콘 순서/위치 동일)과 모바일(390px, 더보기 메뉴 열기/내보내기 하위
  메뉴 진입/항목 선택 후 자동 닫힘)을 모두 확인함.
...
## 2026-09-01 (3) · 교수 전체 코드 등록 폐지 + 상단 툴바에 강의명 표시
- **student-join.js**: 교수 전체 코드(users.prof_code) 폴백 제거 — 이제 classes.code(수업 코드)만 확인. 코드가 없으면 404. 이미 그 수업에 등록돼 있으면 "이미 등록된 강의입니다" 에러.
- **student-professors.js**: 응답에 각 항목별 className 추가(그 교수의 수업 중 내가 실제 속한 수업명, class_students 기준 GROUP_CONCAT — 여러 수업이면 쉼표로 나열, 없으면 null).
- **app.js refreshProfBar**: 상단 툴바 표시를 "교수이름"에서 className이 있으면 "강의명-교수이름"으로 변경(1명일 때/드롭다운 둘 다).
- **auth.js**: 학생 [설정] 등록 목록에도 강의명 함께 표시. 교수 쪽 [등록 코드] 모달은 더 이상 자기 코드를 보여주지 않고 [수업 관리]로 안내하는 문구로 교체. 라벨/문구 전반 "교수 코드"→"강의 코드"로 정리.
- **student-submit.js / app.js**: 남아있던 "교수 코드 입력" 안내 문구를 "강의 코드 입력"으로 정리.
- DB 스키마 변경 없음(users.prof_code 컬럼 자체는 남아있음 — 과거 데이터 호환용, 신규 등록에는 더 이상 쓰이지 않음).

## 2026-09-08 · [긴급] 저장은 되는데 재로그인하면 작품이 사라지는 문제 수정
- **증상**: 새 작품 생성 → "저장됨" 표시 → 로그아웃 + 브라우저 기록 삭제 → 재로그인하면 작품이 없음.
- **원인**: `functions/api/_utils.js wipeIfDue()`(매년 3/1·9/1 서버 정리)는 **GET /api/data에서만** 호출된다.
  2026-09-01 기준일이 지나며 실행됐는데, 삭제 대상 표 목록 중 운영 D1에 아직 만들어지지 않은 표
  (예: 9/1에 추가된 submission_feedback_versions)가 있으면 DELETE에서 예외가 나 **GET 요청 전체가 500**이 됐다.
  저장(POST)은 이 함수를 호출하지 않아 계속 성공 → "저장됨"은 뜨지만, 로그인 시 불러오기는 항상 실패.
  브라우저에 로컬 데이터가 남아 있으면 정상처럼 보이고, 기록을 지우면 "작품이 사라진" 것처럼 보였다.
  (user_data는 삭제 목록에서 예외 발생 지점 뒤라 지워지지 않았음 → 서버에 데이터는 남아 있을 것으로 판단)
- **functions/api/_utils.js**: wipeIfDue 전면 방어 — 표별 DELETE 개별 try/catch, 예외를 밖으로 던지지 않음.
  server_meta 읽기 실패 시 삭제 건너뜀, 기록 쓰기 성공 후에만 삭제, **기록이 전혀 없으면 삭제하지 않고 기준일만 기록**
  (학기 중 배포로 인한 예고 없는 대량 삭제 방지 — 다음 기준일 2027-03-01부터 정상 동작).
- **functions/api/data.js**: GET에서 `try { await wipeIfDue(env); } catch(e){}` — 정리 실패가 조회를 막지 못하게.
- **auth.js**: `serverDataLoaded` 플래그 도입. 서버 조회가 성공(또는 "서버에 데이터 없음" 확인)하기 전에는
  서버 저장을 전면 차단(`serverSaveReady()`) — 빈/오래된 로컬 데이터가 서버를 덮어쓰는 사고 방지.
  조회 실패 시 3초 간격 4회 자동 재시도, 계속 실패하면 alert로 1회 명확히 안내. 저장 실패 시 2초 뒤 1회 자동 재시도.
- **app.js**: 서버 저장이 불가한 상태에서는 "저장됨" 대신 "저장 실패"로 정직하게 표시(탭 점도 빨강).
  forceSaveNow()가 save()를 재사용하도록 정리(계정 태그 LS_OWNER_KEY 누락 수정).
  작품 생성/이름변경/삭제, 신규 계정의 기본 작품("내 첫 작품")은 디바운스 없이 즉시 서버 저장.
...
## 2026-09-12 (2) — 과제 종류 지정 (엉뚱한 형식 제출 막기)

"글쓰기 과제인데 플롯 제출물이 들어왔다"는 문의. 과제 폴더에는 종류 구분이 없었고, 제출물의 종류는
학생이 **어느 탭에서 [제출]을 눌렀는지**로만 정해져서 어떤 과제에든 아무 종류나 들어갈 수 있었다.

- `assignments.type` 컬럼 추가(NULL=지정 안 함=예전처럼 모든 종류 허용). 운영 DB에 컬럼이 없어도
  `ensureAssignmentSchema()`가 첫 API 호출 때 `ALTER TABLE ... ADD COLUMN type TEXT`로 만든다.
  그래도 실패할 경우를 대비해 `a.type`을 읽는 조회는 모두 **예전 쿼리로 되돌아가는 try/catch**를 뒀다
  (2026-09-08 "컬럼 하나 때문에 GET 500" 교훈).
- 교수 [과제 등록]/[과제 설정 변경] 모달에 **과제 종류** 선택 추가. 과제 폴더 목록에도 "글쓰기 과제 ·"
  표시. 기존 과제는 "지정 안 함"이라 그대로 동작한다.
- 학생 제출 모달 — 종류가 지정된 과제는 **그 탭에서만** 목록에 보인다. 숨겨진 개수가 있으면
  "다른 종류로 지정된 과제 N개는 그 탭에서 제출해주세요" 안내를 띄운다.
...
## 2026-09-14 (6) — [버그 수정] "직접 그리기" 버튼 먹통 — 함수 두 개가 실수로 삭제됨

증상: 콘티제작에서 [직접 그리기]를 눌러도 아무 반응이 없음.
원인: 9/14 (4) 그리기 개편에서 `saveStoryboardBlob` 구간을 새 코드로 바꿀 때 끝 경계를 너무 뒤로 잡아,
그 사이에 있던 **`openSizePicker`(칸 크기 고르기)와 `downloadCanvasPng`(PNG 내려받기)까지 함께 지워졌다.**
버튼 → `openSizePicker` 호출 → `ReferenceError: openSizePicker is not defined` → 아무 일도 안 일어남.
`node --check`는 문법만 보므로 통과했고, 그리기 엔진 단독 테스트에서도 이 둘을 부르지 않아 드러나지 않았다.

수정: 두 함수를 예전 커밋(0f16cfe)에서 그대로 되살려 그리기 엔진 바로 앞에 다시 넣었다.
덧붙여 `openDrawPage`가 블록을 목록에서 못 찾으면 알림 후 되돌아가던 것을, 넘겨받은 블록을 그대로 쓰도록 고쳤다
(되돌아가면 역시 "버튼이 먹통"처럼 보인다).

재발 방지: 큰 구간을 통째로 바꿀 때는 바꾸기 전/후 파일에서 **정의된 함수 이름 목록을 뽑아 비교**하고,
...
## 2026-09-15 (4) — 대사 한 칸이 첨삭 화면에서 대사+지문으로 쪼개지던 버그

증상: 학생이 대사 칸 하나에 쓴 글이, 교수 첨삭 화면에서는 첫 줄만 대사 칸이고 나머지 줄은 지문
칸으로 따로 떨어져 보였다.

원인 두 가지.
1. 제출 데이터의 글쓰기 `text`는 지문/대사 칸을 `"\n"`으로 이어붙인 한 덩어리이고, 첨삭 화면
   (`renderStyledBeforeText`)은 그 텍스트를 **줄 단위로 다시 쪼개** `"이름: "`으로 시작하면 대사,
   아니면 지문으로 그렸다. 그래서 대사 한 칸 안에 개행이 있으면 둘째 줄부터 지문이 됐다.
2. 대사 입력칸(`.dlg-text`, contenteditable)은 줄바꿈을 그대로 보여주지 않아서(한 줄로 보임)
   엔터·붙여넣기로 들어온 개행이 **학생 눈에는 안 보인 채 데이터에만** 남았다. 학생은 "지워서
   한 줄로 만들었다"고 알고 있는데 서버로는 개행이 함께 갔다.

수정.
- `buildSubmissionData("write")`: 칸 구조를 `items:[{type,char,text}]`로 함께 보낸다.
  `text`는 예전과 똑같이 그 items를 `"\n"`으로 이어붙인 값이라 [내 작업물에 반영] 등 기존 기능은 그대로.
- `writeItemSegments(items, rawText)` 신규: items를 rawText 안의 글자 구간으로 되돌린다. 되돌린 전체
  길이가 rawText와 한 글자라도 다르면 `null` → 옛 제출물은 예전처럼 줄 단위로 그린다(안전장치).
- `renderStyledBeforeText(..., items)`: 구간이 있으면 **학생이 만든 칸 그대로** 한 칸씩 그린다.
  메모(하이라이트·각주)는 글자 위치로 저장되므로 DOM 안 텍스트 합이 rawText와 정확히 같아야 하는데,
  칸 사이 개행을 `.nl-keep`으로 넣어 그대로 맞춘다(jsdom으로 글자수 일치·하이라이트 구간 확인).
- `buildReviewPairs("write")`가 `items`를 pair에 실어 `renderBeforeBody`로 넘긴다.
- 대사 입력칸: 엔터는 편집 끝내기(`blur`)로 바꾸고, 붙여넣기 등으로 들어온 개행은 공백으로 정규화
  (`it.text=tx.textContent.replace(/[\r\n]+/g," ")`). 지문 칸(textarea)의 여러 줄은 그대로 둔다.

이미 제출된 옛 자료는 items가 없어 예전처럼 보이지만, 다시 제출하면 칸 그대로 넘어간다.

## 2026-09-15 (5) — 제출 차수 목록에 시간 표시

같은 날 여러 번 제출하는 경우가 많아 날짜만으로는 차수를 구분하기 어려워서, 차수 목록에는
날짜 뒤에 시:분을 함께 보여준다.
...
## 2026-09-17 — 콘티 첨삭 화면에 메모 + "전체 평가"

- 칸마다 [메모 추가] 버튼(우클릭 메뉴도 가능). 지문·대사를 드래그해 선택한 채 누르면 그 부분에 색 하이라이트+각주 번호,
  선택 없이 누르면 칸 전체 메모(*). 메모 카드는 클릭해서 수정, ✕로 삭제. pairId = 콘티 칸 id.
- 대본 표시는 `sbScriptBlock(b, counter, mm)`이 mm(메모 번호표)이 있을 때 글쓰기 첨삭과 같은
  `renderStyledBeforeText`로 그리고, 대사 번호는 `data-memo-synthetic` span으로 끼워 글자 위치 계산에서 뺀다.
- 메모 저장: 그림 저장(`submitStoryboardFeedback`)에 현재 메모를 함께 보내고(예전엔 `memos:[]`로 지워짐),
  [피드백 전달] 때 메모가 바뀌었으면 지금 그림 상태(없으면 `sbPassthroughFeedback`)로 새 버전을 만들어 저장한다.
- 그리기 페이지에 다녀와도 메모가 남도록 `sbReviewMemoDraft`에 보관. 전달 안 한 메모가 있으면 [제출함으로]에서 확인창.
- 평가 입력창 이름을 콘티·파일 과제에서는 "전체 평가"로 표시. 학생 [첨삭 보기]에서도 메모가 읽기 전용으로 보인다.
- 서버 변경 없음.

## 2026-09-17 (2) — [평가 저장] 버튼 제거

첨삭 화면의 평가(전체 평가)는 [피드백 전달]을 누를 때만 함께 저장·전달된다(서버는 그대로).
