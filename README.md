...
- 서버 데이터는 매년 3월 1일·9월 1일(6개월 주기)에 계정 정보만 남기고 초기화됩니다(로컬 저장본은 유지)
...
`/api/data` 조회 시점에 매년 3월 1일·9월 1일 기준일이 지났으면(6개월 주기) users를 제외한 서버 데이터가
전부 초기화됩니다(`functions/api/_utils.js`의 `wipeIfDue()`, 같은 반기 동안은 한 번만 실행됨).
